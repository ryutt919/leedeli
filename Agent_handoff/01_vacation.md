...
### Quick Actions Row (탭 콘텐츠 최상단 빠른 작업 행)
- **Current value/logic**: Right below the outer tab selector (i.e. the very first element `VacationTabContent` renders, above "직원별 휴가 현황"), a single horizontal `<Space wrap>` row holds four small (`size="small"`) buttons in this order: "전체 월차 부여" / "전체 연차 부여" (primary, open the bulk-grant modal — see Bulk Grant below) and "부여 기록 초기화" / "사용 기록 초기화" (danger, wrapped in `Popconfirm` — see Bulk Reset below). All four sit on one line (`wrap` only kicks in on very narrow viewports) instead of being split across two separate cards with full-width stacked buttons.
- **Implementation**: Plain `<Space wrap size={8}>` + `<Button size="small" .../>` / `<Popconfirm><Button size="small" danger .../></Popconfirm>` — no new state; reuses `bulkGrantForm`/`setBulkGrantType`/`grantLoading` (grant buttons) and `handleResetAllGrants`/`handleResetAllUsage`/`allResetLoading` (reset buttons) from the sections below.
- **Related files**: `src/pages/VacationPage.tsx`
- **Rationale**: User asked to consolidate all four all-employee actions into one compact row of small buttons placed directly beneath the tab selector ("네개 다 작은 버튼으로 1열로 탭 선택 아래에 놔두고 싶어") instead of two separate cards each containing full-width stacked buttons — saves vertical space and groups all "전체 직원 대상" actions visually.

### Leave Grant Rules → Bulk Grant (removed rule engine)
- **Current value/logic**: The old `LeavePolicy`-based automatic grant system (월/년/n일 주기 규칙 + 자동 부여 재실행) has been fully removed. In its place, the quick-actions row (see above) has two buttons — "전체 월차 부여" and "전체 연차 부여" — each opening its own single-field modal titled `전체 정직원에게 {월차|연차} 부여` (only that type's day amount, no mixed 월차+연차 form). Confirming grants that amount to every 정직원 for the current month (월차, `grant_month='YYYY-MM'`) or current year (연차, `grant_month='YYYY'`), skipping anyone already granted for that period.
...
| 2026-06-06 (레이아웃 통합) | 전체 작업 버튼 4개를 한 줄로 통합 | "네개 다 작은 버튼으로 1열로 탭 선택 아래에 놔두고 싶어" 요청에 따라 "휴가 일괄 부여"/"전체 직원 휴가 초기화" 두 개의 카드(각 풀폭 버튼 2개씩 세로 배치) → 단일 `<Space wrap size={8}>` 가로 행(`size="small"` 버튼 4개: 전체 월차 부여 / 전체 연차 부여 / 부여 기록 초기화 / 사용 기록 초기화)으로 교체, `VacationTabContent` 최상단(탭 선택 바로 아래, "직원별 휴가 현황" 카드보다 위)으로 이동; 버튼 라벨도 "전체 정직원에게 ~"/"전체 직원 ~" → "전체 ~"/"~ 초기화"로 축약(한 줄에 맞도록); 핸들러/상태(`handleBulkGrantByType`, `handleResetAllGrants`, `handleResetAllUsage` 등)는 변경 없이 재사용 |
