import type { Person, DayAssignment, Schedule, ShiftType, ValidationError, Prep, Ingredient } from './types';
...
export function validateGeneratedSchedule(schedule: Schedule): ValidationError[] {
    ...
    schedule.assignments.forEach((day: DayAssignment) => {
        const requiredStaff = dailyStaffByDate[day.date] ?? rules.DAILY_STAFF_BASE;
        const requiredUnits = toHalfUnits(requiredStaff);

        const halfCount = day.people.filter((p: any) => p.isHalf).length;
        const fullCount = day.people.filter((p: any) => !p.isHalf).length;
        const assignedUnits = fullCount * 2 + Math.floor(halfCount / 2) * 2;

        const openCount = day.people.filter((p: any) => p.shift === 'open').length;
        const middleCount = day.people.filter((p: any) => p.shift === 'middle').length;
        const closeCount = day.people.filter((p: any) => p.shift === 'close').length;
        ...
        schedule.people.forEach(person => {
            ...
            const assigned = day.people.find((p: any) => p.personId === person.id);
            if (!assigned) {
                errors.push({
...
export function generateSchedule(
    ...
    for (let date = 1; date <= daysInMonth; date++) {
        ...
        const fixedHalfCount = dayAssignment.people.filter((p: any) => p.isHalf).length;
        ...
        if (mustOpenPeople.length > 0) {
            ...
            const already = dayAssignment.people.find((p: any) => p.personId === person.id);
            if (!already) {
                dayAssignment.people.push({
                    personId: person.id,
                    personName: person.name,
        ...
        if (mustClosePeople.length > 0) {
            ...
            if (!dayAssignment.people.find((p: any) => p.personId === person.id)) {
                dayAssignment.people.push({
                    personId: person.id,
                    personName: person.name,
                    shift: 'close',
        ...
        const alreadyAssignedFull = new Set(dayAssignment.people.filter((p: any) => !p.isHalf).map((p: any) => p.personId));
        const remainingPeople = availableFullPeople.filter(p => !alreadyAssignedFull.has(p.id));

        while (dayAssignment.people.filter((p: any) => !p.isHalf).length < requiredFullCount) {
            const openCountAll = dayAssignment.people.filter((p: any) => p.shift === 'open').length;
            const middleCountAll = dayAssignment.people.filter((p: any) => p.shift === 'middle').length;
            const closeCountAll = dayAssignment.people.filter((p: any) => p.shift === 'close').length;
        ...
        const halfCountAfter = dayAssignment.people.filter((p: any) => p.isHalf).length;
        const fullCountAfter = dayAssignment.people.filter((p: any) => !p.isHalf).length;
        const assignedUnitsAfter = fullCountAfter * 2 + Math.floor(halfCountAfter / 2) * 2;

        const openCount = dayAssignment.people.filter((p: any) => p.shift === 'open').length;
        const middleCount = dayAssignment.people.filter((p: any) => p.shift === 'middle').length;
        const closeCount = dayAssignment.people.filter((p: any) => p.shift === 'close').length;
        ...
        people.forEach(person => {
            const requestedShift = person.halfRequests?.[date];
            if (requestedShift === undefined) return;
            const assigned = dayAssignment.people.find((p: any) => p.personId === person.id);
            if (!assigned) {
                generationErrors.push({
                    type: 'half-not-assigned',
...
export function exportSchedulesToExcelCsv(schedules: Schedule[]): void {
    ...
    schedules.forEach((schedule, scheduleIndex) => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (assignment) {
                assignment.people.forEach((p: any) => {
