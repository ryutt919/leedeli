...
import { getScheduleById, saveSchedule, saveStaffConfig, loadStaffConfig } from '../../v2/storage';
import { getWorkRules } from '../../v2/workRules';
...
export function CreateSchedulePageV2() {
    ...
    useEffect(() => {
        ...
        const id = sp.get('editId') || (location.state as { editScheduleId?: string } | null)?.editScheduleId;
        if (!id) return;
    ...
    useEffect(() => {
        const sp = new URLSearchParams(location.search);
        if (sp.get('editId') || (location.state as { editScheduleId?: string } | null)?.editScheduleId) return; // 편집 모드면 자동 로드하지 않음
